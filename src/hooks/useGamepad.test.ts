...
describe('useGamepad', () => {
  beforeEach(() => {
    ...
    global.navigator.getGamepads = jest.fn(() => [
      { id: 'Gamepad 1', index: 0, buttons: [], axes: [] },
  ...
  it('calls didConnect when a gamepad is connected', () => {
    ...
    renderHook(() => useGamepad({ didConnect: mockDidConnect, didDisconnect: mockDidDisconnect, didUpdate: mockDidUpdate }));
    ...
    expect(mockDidConnect).toHaveBeenCalledWith(expect.objectContaining({ id: 'Gamepad 1', index: 0 }));
  });
