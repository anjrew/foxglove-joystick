...
export const useGamepadState = (layoutName: string, cbInteractChange: (joy: Joy) => void) => {
  ...
  useEffect(() => {
    if (displayMapping.length === 0) {
      ...
      setNumButtons(
        Math.max(
          ...displayMapping.map((item) => (item.type === "button" ? (item as ButtonConfig).button : -1))
        ) + 1
      );
      setNumAxes(
  ...
  const buttonCb = useCallback((idx: number, e: React.PointerEvent, eventType: PointerEventType) => {
    switch (eventType) {
      case PointerEventType.Down:
        e.currentTarget.setPointerCapture(e.pointerId);
        setInteractions((interactions) => [
          ...interactions,
          { pointerId: e.pointerId, buttonIdx: idx, buttonVal: 1, axis1Idx: -1, axis1Val: -1, axis2Idx: -1, axis2Val: -1 },
        ]);
        break;
      case PointerEventType.Move:
        break;
      case PointerEventType.Up:
        setInteractions((interactions) => interactions.filter((i) => i.pointerId !== e.pointerId));
        break;
    }
  }, []);

  const axisCb = useCallback((idxX: number, idxY: number, e: React.PointerEvent, eventType: PointerEventType) => {
    ...
    switch (eventType) {
      case PointerEventType.Down:
        ...
        setInteractions((interactions) => [
          ...interactions,
          { pointerId: e.pointerId, buttonIdx: -1, buttonVal: -1, axis1Idx: idxX, axis1Val: xa, axis2Idx: idxY, axis2Val: ya },
        ]);
        break;
      case PointerEventType.Move:
        setInteractions((interactions) =>
          interactions.map((v) => (v.pointerId === e.pointerId ? { ...v, axis1Idx: idxX, axis1Val: xa, axis2Idx: idxY, axis2Val: ya } : v))
        );
        break;
      case PointerEventType.Up:
        setInteractions((interactions) => interactions.filter((i) => i.pointerId !== e.pointerId));
